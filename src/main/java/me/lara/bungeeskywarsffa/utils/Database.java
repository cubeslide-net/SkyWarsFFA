...
public class Database {
  ...
  public Database() {
    ...
    hikariConfig.setJdbcUrl(
        "jdbc:mysql://" + spigotConfig.getString("MYSQL.HOSTNAME") + ":" + spigotConfig.getInt(
            "MYSQL.PORT") + "/" + spigotConfig.getString("MYSQL.DATABASE") + "?autoReconnect=true");
  ...
  public boolean doesPlayerExistByUUID(UUID uuid) {
    try {
      final PreparedStatement prepareStatement = getConnection().prepareStatement(
          "SELECT * FROM `SkyWarsFFAStats` WHERE UUID=?;");
  ...
  public void createTable() {
    try {
      ...
      PreparedStatement ps = conn.prepareStatement(
          "CREATE TABLE IF NOT EXISTS `SkyWarsFFAStats` " + "("
              + "UUID varchar(40), KILLS int, DEATHS int" + ");");
      ps.executeUpdate();
    } catch (SQLException e) {
      e.printStackTrace();
    }
  ...
  public int getKills(UUID uuid) {
    try {
      ...
      final PreparedStatement preparedStatement = connection.prepareStatement(
          "SELECT * FROM SkyWarsFFAStats WHERE uuid = '" + uuid + "';");
  ...
  public void addDeath(UUID uuid) {
    try {
      ...
      PreparedStatement prepareStatement = connection.prepareStatement(
          "UPDATE SkyWarsFFAStats SET DEATHS=" + (getDeaths(uuid) + 1) + " WHERE UUID='"
              + uuid.toString() + "'");
  ...
  public void addKill(UUID uuid) {
    try {
      Connection connection = getConnection();
      PreparedStatement prepareStatement = connection.prepareStatement(
          "UPDATE SkyWarsFFAStats SET KILLS=" + (getKills(uuid) + 1) + " WHERE UUID='"
              + uuid.toString() + "'");
      prepareStatement.execute();
    } catch (Exception exception) {
      exception.printStackTrace();
  ...
  public void createNewUser(UUID uuid) {
    ...
    try {
      Connection connection = getConnection();
      PreparedStatement prepareStatement = connection.prepareStatement(
          "INSERT INTO SkyWarsFFAStats(UUID, KILLS, DEATHS) VALUES(?, ?, ?);");
      prepareStatement.setString(1, uuid.toString());
